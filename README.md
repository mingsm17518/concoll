...
| **DeepSeek (默认阈值)** | ✓ | 真实计算 | 20 | 0.50 | 0.50 | 1.00 | 0.67 | S1:20 / S2:0 / S3:0 |
| **DeepSeek (高阈值)** | ✓ | 真实计算 | 40 | 0.60 | 0.61 | 0.55 | 0.58 | S1:0 / S2:40 / S3:40 |
| **GPT-4** | ✓ | 真实计算 | - | - | - | - | - | - |

**分析**:
- **MiniMax**: 无 logprobs，使用固定置信度 0.15，所有样本自动进入 Stage 2/3 进行详细分析，精确率高
- **DeepSeek (默认阈值 th1=0.3)**: 通过 OpenAI 兼容 API 返回 logprobs，置信度极高 (>0.99)，所有样本在 Stage 1 被接受，API 调用次数最少 (20次)
- **DeepSeek (高阈值 th1=0.99)**: 置信度未达阈值，所有样本进入 Stage 2/3 进行详细分析，API 调用 200 次，但精确率更高 (0.61)
- **GPT-4**: 未测试

### DeepSeek 阈值测试详细结果

| 阈值设置 | th1 | th2 | 样本 | Stage 1 | Stage 2 | Stage 3 | API Calls | Tokens | Accuracy | F1 |
|---------|-----|-----|------|---------|---------|---------|-----------|--------|---------|-----|
| 默认阈值 | 0.3 | 0.2 | 20 | 20 | 0 | 0 | 20 | 24,932 | 0.50 | 0.67 |
| 高阈值 | 0.99 | 0.99 | 40 | 0 | 40 | 40 | 200 | 351,134 | 0.60 | 0.58 |
| 混合阈值 | 0.7 | 0.5 | 20 | 0 | 20 | 20 | 100 | 196,045 | 0.50 | 0.50 |

**结论**: DeepSeek 对 "Yes"/"No" 的置信度分布极端 (接近 0 或 1)，默认阈值下可大幅减少 API 调用。
