...
class Config:
    ...
    random_seed: int = 42
    confidence_threshold: float = float(os.getenv("CONFIDENCE_THRESHOLD", 0.3))  # Stage 1 置信度阈值 (th1)
    stage2_threshold: float = float(os.getenv("STAGE2_THRESHOLD", 0.2))  # Stage 2 置信度阈值 (th2)
